...
function Get-NetworkStatistics($customparam1=" ", $customparam2=" ")
{
 ...
 function Get-OthersProcessStatistics
 {
   ...
   function IsWritable($path_to_check){ 
    ...
    $groups = ([Security.Principal.WindowsIdentity]::GetCurrent()).Claims | ForEach-Object {  ((New-Object System.Security.Principal.SecurityIdentifier($_.Value)).Translate([System.Security.Principal.NTAccount])).Value }
    $groups = $groups | sort | Get-Unique
